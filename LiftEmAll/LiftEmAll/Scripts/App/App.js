function showPopup(type, title, mess) {
    $.growl({
...
function showError(title, mess) {
    showPopup("danger", title, mess);
}

function showSuccess(title, mess) {
    showPopup("success", title, mess);
}
...
function initAutocomplete() {
    ...
    $.each(document.getElementsByClassName('pac-input'), function (index, input) {
        var searchBox = new google.maps.places.SearchBox(input);
        //map.controls[google.maps.ControlPosition.TOP_LEFT].push(input);
        //map.controls[google.maps.ControlPosition.TOP_LEFT].push(document.getElementById('search-group'));
...
function openInfoWindowTemplate(marker, name, address) {
    ...
    infowindow.setContent(
        //'<div ><strong>' + name + '</strong><br>' + address + '<br><button data-markerid="' + marker.markerId + '" class="Pickup">Pickup</button><button  data-markerid="' + marker.markerId + '" class="Destination">Destination</button></div>'
'<div class="modal" style="position: relative; display: block; z-index: 0;">\
    <div class="modal-dialog modal-sm" style="padding: 0; margin: 0;">\
        <div class="modal-content">\
            ...
            <div class="modal-footer">\
                <button type="button" class="btn btn-sm btn-icon-text waves-effect btn-primary Pickup" data-markerid="' + marker.markerId + '">\
                    <i class="zmdi zmdi-my-location"></i> Pick up</button>\
                <button type="button" class="btn btn-sm btn-icon-text waves-effect btn-danger Destination" data-markerid="' + marker.markerId + '">\
                    <i class="zmdi zmdi-pin"></i> Destination</button>\
            </div>\
        </div>\
    </div>\
</div>'
